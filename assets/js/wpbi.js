...
jQuery(document).ready(function($){
	...
	if ($('select#product-type').val() == "wpbi"){
		$('.show_if_neo_wpbi_options').show();
		$('#campaign-update-status-meta').show();
	}
	...
	$('body').on('click','.wpneo-image-remove',function(e) {
		var that = $(this);
	    $(that).parent().parent().find( '.wp_product_banner_image_image_field' ).val( '' );
	...
	$(document).on('click', 'a.wpneo-wpbi-reset-btn', function(){
		...
		$.ajax({
			type : 'POST',
			url : ajaxurl,
			data : { action : 'wpbi_settings_reset'},
	...
	$(document).on('change', '.wpbi_addons_list_item', function(e) {
        var $that = $(this);
        ...
        $.ajax({
            url : ajaxurl,
            type : 'POST',
            data : {isEnable:isEnable, addonFieldName:addonFieldName, action : 'wpbi_addon_enable_disable'},
            success: function (data) {
                if (data.success){
                    //Success
                }
